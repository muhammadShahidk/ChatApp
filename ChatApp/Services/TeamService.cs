        ...
        private Team CreateOverflowTeam()
        {
            ...
            for (int i = 1; i <= 6; i++)
            {
                overflowAgents.Add(new Agent
                {
                    ...
                    Status = AgentWorkStatus.Offline, // Start offline
                    TeamId = "OVERFLOW",
                    IsOverflowTeam = true,
                    CurrentShift = GetCurrentShift(),
                    ShiftStartTime = GetShiftStartTime(),
        ...
        public void UpdateAgentShiftStatus()
        {
            ...
            foreach (var team in _teams)
            {
                foreach (var agent in team.Agents)
                {
                    ...
                    {
                        if (agent.Status == AgentWorkStatus.Available)
                        {
                            agent.Status = AgentWorkStatus.ShiftEnding;
                        }
                    }
                    ...
                    else if (currentTime >= agent.ShiftEndTime)
                    {
                        ...
                        {
                            agent.Status = AgentWorkStatus.Offline;
                        }
                    }
